...
class NodePosition():
	...
	self.y =np.linspace(-2*math.pi,2*math.pi,len(self.t)+1)
	self.xrd =[3*math.cos(self.yy)/(1 + math.sin(self.yy)**2) for self.yy in self.y]  ;
	self.yrd = [3*math.cos(self.yy)*math.sin(self.yy)/(1 + math.sin(self.yy)**2) for self.yy in self.y] ;	
